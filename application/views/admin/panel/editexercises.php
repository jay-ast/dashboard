	                  ...
	                  {{message}}
                        <div class="col-md-12">
	                                ...
	                                <div id = "divMain">
	                                    <div id="Dragexercise" class="droppable">
	                                      <div v-for="(item,index) in assignVideos" :key="item.id" :id="`Exreciseid${item.id}`" :dataid="item.id" class="draggable">
		                                    <a class="badge pull-left" style="color:white">{{index + 1}}</a>  <div class="width-100">{{item.name}}</div><a :dataid="item.id" @click="deletevideo(item.id)" class="deleteVideo pull-right"><i class="glyphicon glyphicon-trash "></i></a></div>
	                                    </div>
                         ...
                         <?PHP
	                        if($adding){
		                     ?>
		                      <div id="checkSaveRoutine" class="pull-right btn btn-success" >Save Routine</div>
		                      <button  type="submit" id="btnSaveExercise" style="display:none;" class="pull-right btn btn-success btnSaveExercise" >Save Routine</button>

		                     <?PHP   
	                        }
	                 else{
?>
 <div class=" btn btn-success submit-form saveAsNew" url="<?php echo base_url('admin/exercises/addNewExercise') ?>">Save as New</div> &nbsp;&nbsp;&nbsp;
 <button id="btnSaveasnew" style="display:none;" type="submit"></button>
  <div id="btnupdateuser" class=" btn submit-form btn-success btnupdateuser" url="<?php echo base_url('admin/exercises/UpdateExercise') ?>">Update</div>
  <div id="btnUp" @click="goUp" class="pull-left"><img src="/assets/custom/images/arrow_up.png" style="
    height: 45px;
    position: absolute;
    right: 7px;
    bottom: 1px;
...
<?PHP
    ...
    <div class="row">
	    <div data-id="222222" class="col-md-6">
                ...
                <img class="<?php echo ($isgeneral==true)?'hidden':'' ?>" onclick="showMe(this)" data-toggle="yes" src="/assets/custom/images/showme.jpg" style="height: 25px;float: left;margin-right: 8px;cursor: pointer;" /><h4>General Routines Videos</h4>           
              ...
              <div data-id="11111111"  class="col-md-6">
                      ...
                      <div  class="clientvideoslist videolist ">
                        ...
                        <div v-else class="form-group col-md-10 <?php echo ($isgeneral==true)?'hidden':'' ?>">
	                     No Patient Video Found
	                    <a :href="`/admin/folders/detail/${companyVideoFolderId}?ref=${path}`"> <div class="btn btn-success">Upload Video</div></a>
                     	</div>
...
<div id="generalvideosfolder" v-if="folderName" class="row">  
	...
	<h3>{{folderName}}<span class="badge" style="color: white;margin-left: 7px;padding-top: -11px;">{{currentRoutineCount}}</span></h3>
		...
		<div class="row" >  
			...
			<div v-for="(item,index) in arrGeneralVideos" 
			:key="index" 
			v-show="filterVideoName == '' || item.title.toLowerCase().indexOf(filterVideoName.toLowerCase()) > -1" 
			class="myroutineselector col-xs-6 col-md-4 col-lg-4">
			<div class="widget widget-inverse">
...
<div v-if="clientVideos" class="publicvideoslist videolist <?php echo ($isgeneral==true)?'hidden':'' ?>">
	...
	<h3>{{clientSelect}} Videos</h3>
		<div class="row">  
			...
			<div v-for="(item,index) in clientVideos" 
			:key="index" 
			v-show="filterClientVideoName == '' || item.title.toLowerCase().indexOf(filterVideoName.toLowerCase()) > -1" 
			class="myroutineselector col-xs-6 col-md-4 col-lg-4">
			<div class="widget widget-inverse">
				<div @click="selectRoutineVideo(item)" :class="`widget-head routineselector ${item.selected}`" :id="`video-${item.id}`" :data-videoid="item.id" :data-videoname="item.title">{{item.title}}</div>
					<div data-id="1111111" class="widget-body padding-none text-center">
...
<?php if (!empty($assginedvideosname)) {
    foreach ($assginedvideosname as $key=>$name) {
        ?>
            ...
            selectedVideos.push({name: $vname, id: $vid})
        <?php
    }
...
*/
			  ...
			  selectRoutineVideo: function (item) {
				...
				var videoname=item.title;	
		         ...
		         { 
		            self.assignVideos.push({id: item.id, name: videoname})
